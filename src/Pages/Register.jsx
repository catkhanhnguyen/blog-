  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post(baseUrl, {
        username: username,
        password: password,
        role: role // Thêm role vào request body
      });
      if (response.status === 201) { // Sử dụng status 201 để xác nhận user đã được tạo thành công
    ...
    <div>
      ...
      <div className="bg-yellow-400 dark:bg-gray-800 h-screen overflow-hidden flex items-center justify-center">
        <div className="bg-white lg:w-6/12 md:7/12 w-8/12 shadow-3xl rounded-xl">
          ...
          <form className="pt-12 pb-8 px-12 md:p-24" onSubmit={handleRegister}>
            ...
            <div className="flex items-center text-lg mb-6 md:mb-8">
              ...
              <select
                id="role"
                className="bg-gray-200 rounded py-2 md:py-4 focus:outline-none"
                value={role}
